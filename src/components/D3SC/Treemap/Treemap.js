...
function Treemap({ expandedCard }) {
  ...
  useEffect(() => {
    ...
    .attr('height', height).attr('width', width).style('border', '1px solid  black')
      ...
      legend.attr('transform', `translate(0, 30)`)
        ...
        cell.append('text') // Adding text in tspans within the rectangles/cells/leaves
          ...
          .style('font-size', function(d, i, nodes) {
            ...
            return Math.min(parentData.x1 - parentData.x0, parentData.y1 - parentData.y0) / 8; // Adjust font size based on rectangle size
            });
  ...
  }, [currentDataset, expandedCard]);

  return (
    <div className="hm-container">
      {expandedCard !== 5 && (
        <>
        <h3 style={{color: 'var(--orange)'}}>Only Available When Expanded</h3>
        </>
      )}  
      {expandedCard === 5 && (
        <>
        <button onClick={toggleDataset} className='hm-btn'>{buttonText}</button>
        <h3 className='hm-title'>{title}</h3>
        <h3 className='hm-description'>{description}</h3>
        <div ref={divRef}></div> 
        </>
      )}  
